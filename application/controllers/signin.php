  ...
  public function index() {
    if ($this->session->userdata('logged_in') == TRUE) {
      if ($this->session->userdata('usr_access_level') == 1) {
        ...
        redirect('me'); 
        //redirect('bienvenido/usuarios');
      }  
    } else {
      // Set validation rules for view filters. No hay sesion activa
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        if ($query->num_rows() == 1) { // One matching row found
          foreach ($query->result() as $row) {
            ...
            if ($row->usr_is_active != 0) { // See if the user is active or not
              ...
              if ($hash != $row->usr_hash) {
                ...
                $data = array(
                    'usr_id' => $row->usr_id,
                    //'acc_id' => $row->acc_id,  ME DA ERROR POR ESO LO COMENTO,VER SI MAS ADELANTE LO USO
          ...
          // aca no encontro ningun usuario
          $data = array("mensaje" => true,
          );
           $this->load->view('common/header');
           $this->load->view('users/signin', $data);
           $this->load->view('common/footer');
